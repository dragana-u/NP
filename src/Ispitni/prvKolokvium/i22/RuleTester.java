package Ispitni.prvKolokvium.i22;
    ...
    public static void main(String[] args) {
        ...
        if (testCase == 1) { //Test for String,Integer
            ...
            * */
            ...
            rules.add(new Rule<>(r -> r.grades.size()>=3, student -> (double)student.grades.stream().mapToInt(i -> i).max().getAsInt()));
            ...
            rules.add(new Rule<>(r -> r.id.startsWith("20"),r -> r.grades.stream().mapToDouble(i -> i).average().orElse(5.0)));
...
class RuleProcessor{
    static <I,O> void process(List<I> vlezni, List<Rule<I,O>> pravila){
        vlezni.forEach(input ->{
            System.out.printf("Input: %s%n",input);
            pravila.forEach(rule ->{
                Optional<O> res = rule.apply(input);
                res.ifPresent(r -> System.out.printf("Result: %s%n",r));
                if(res.isEmpty()){
