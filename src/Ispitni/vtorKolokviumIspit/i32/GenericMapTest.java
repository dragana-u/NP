package Ispitni.vtorKolokviumIspit.i32;
...
class MapOps{
    ...
    public static <K,V> Map<K,V> merge(Map<K, V> mapLeft, Map<K, V> mapRight, MergeStrategy<V> mergeStrategy) {
        Map<K,V> res = new TreeMap<>();
        mapLeft.forEach((k,v) -> res.merge(k,v,mergeStrategy::merge));
        mapRight.forEach((k,v) -> res.merge(k,v,mergeStrategy::merge));
        return res;
    }
}
...
public class GenericMapTest {
    public static void main(String[] args) {
        ...
        if (testCase == 1) { //Mergeable integers
            ...
            MergeStrategy<String> mergeStrategy = (x,y) -> x.replace(y,"*".repeat(y.length()));
            printMap(MapOps.merge(mapLeft, mapRight, mergeStrategy));
        }
    }
