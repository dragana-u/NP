package Ispitni.vtorKolokviumIspit.i5;
...
class GenericCollection<T extends Comparable<T> & IHasTimestamp> {
    ...
    void addGenericItem (String category, T element){
        Set<T> i = items.get(category);
        if(i==null){
            i = new TreeSet<>();
        }
        i.add(element);
        items.putIfAbsent(category,i);
    }
    Collection<T> findAllBetween (LocalDateTime from, LocalDateTime to){
        return items.values().stream().flatMap(Set::stream).filter(i -> i.getTimestamp().isAfter(from) && i.getTimestamp().isBefore(to)).sorted(Comparator.reverseOrder()).collect(Collectors.toList());
    }
    Collection<T> itemsFromCategories (List<String> categories){
        List<List<T>> res = new ArrayList<>();
        for (String category : categories) {
            if(items.containsKey(category)){
                res.add(new ArrayList<>(items.get(category)));
            }
        }
        List<T> collect = res.stream().flatMap(Collection::stream).sorted(Comparator.reverseOrder()).collect(Collectors.toList());
        return collect;
    }
    public Map<String, Set<T>> byMonthAndDay(){
        return items.values()
                .stream().flatMap(Collection::stream)
                .collect(Collectors.groupingBy(i -> String.format("%02d-%02d",i.getTimestamp().getMonth().getValue(), i.getTimestamp().getDayOfMonth())
                        ,TreeMap::new
    ...
    public Map<Integer, Long> countByYear(){
        return items
                .values()
                .stream().flatMap(Collection::stream)
                .collect(Collectors.groupingBy(i -> i.getTimestamp().getYear(),
                        TreeMap::new,
                        Collectors.counting()));
    }
}
