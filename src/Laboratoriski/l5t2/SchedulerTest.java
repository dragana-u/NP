...
class Timestamp<E> implements Comparable<Timestamp<? extends E>> {
    final LocalDateTime time;
    final E element;

    Timestamp(LocalDateTime time, E element) {
        this.time = time;
    ...
    @Override
    public int compareTo(Timestamp<? extends E> o) {
...
class Scheduler<T>{
    ...
    boolean remove(Timestamp<T> t){
        return timestamps
                .removeIf(i -> i.equals(t));
    }
    Timestamp<T> next(){
        return timestamps
                .stream().sorted()
                .filter(i -> i.time.isAfter(LocalDateTime.now()))
                .findFirst().orElse(null);
    }
    Timestamp<T> last(){
        return timestamps
                .stream().sorted(Comparator.reverseOrder())
                .filter(i -> i.time.isBefore(LocalDateTime.now()))
                .findFirst().orElse(null);
    }
    List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end){
        return timestamps
                .stream().sorted()
                .filter(i -> i.time.isAfter(begin) && i.time.isBefore(end))
