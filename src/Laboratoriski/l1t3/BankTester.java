...
class Account {
    ...
    public void setBalance(String saldo) {
        this.saldo = saldo;
    }
    ...
    public String toString() {
        ...
        stringBuilder.append(String.format("Balance: %s\n", saldo));
    ...
    public boolean equals(Object o) {
        ...
        return ID == account.ID && Objects.equals(name, account.name) && Objects.equals(saldo, account.saldo);
    }

    @Override
    public int hashCode() {
        return Objects.hash(name, ID, saldo);
    }
}
...
class FlatPercentProvisionTransaction extends Transaction {
    ...
    public String getProvision() {
        ...
        amount = amount / 100;
        long provision = centsPerDolar * amount;
...
class Bank {
    ...
    public boolean makeTransaction(Transaction t) {
        Account fromAcc = findAccountById(t.fromId);
        Account toAcc = findAccountById(t.toId);
        if (fromAcc != null && toAcc != null) {
            long fromSaldo = toNumber(fromAcc.getBalance());
            long toSaldo = toNumber(toAcc.getBalance());
            long amount = toNumber(t.getAmount());
            long provision = toNumber(t.getProvision());
            if (provision + amount <= fromSaldo) {
                if (fromAcc.getId() == toAcc.getId()) {
                    fromSaldo = toSaldo = (fromSaldo - provision);
                } else {
                    fromSaldo -= provision + amount;
                    toSaldo += amount;
                }
                fromAcc.setBalance(toString(fromSaldo));
                toAcc.setBalance(toString(toSaldo));
    ...
    void updateTotals(long amount, long provision) {
        long totalTransf = toNumber(this.totalTransfers);
        long totalProv = toNumber(this.totalProvision);
        totalTransf += amount;
        totalProv += provision;
        this.totalTransfers = toString(totalTransf);
        this.totalProvision = toString(totalProv);
    }
    ...
    public String toString() {
        ...
        Arrays.stream(accounts)
                .forEach(each -> stringBuilder.append(each.toString()));
        return stringBuilder.toString();
    }
